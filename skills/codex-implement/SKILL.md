...
- Model/effort defaults live in `codex-plan-review/scripts/_common.sh` (implementation → gpt-5.6-luna, reviews → gpt-5.6-sol; derived from `STATE_DIR`). Adjust that one file to your preferred models, or override per run via `CODEX_MODEL` / `CODEX_EFFORT` env vars; the scripts echo the effective values.
- **Cost tiering**: the orchestrator sets `CODEX_TIER=simple|standard|complex` per batch (from the plan's `[S]`/`[M]`/`[C]` markers). `simple` downshifts to `CODEX_MODEL_LIGHT` (if set) at medium effort, `standard` runs the flow model at high effort, `complex` (the default when unset) runs frontier at xhigh. Set `CODEX_MODEL_LIGHT` in your environment once your provider's cheap coding model is known.
